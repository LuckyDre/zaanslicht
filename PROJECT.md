...
_Laatste update: 26 juli 2026 — v0.47_
...
- 📸 **fotograaf.html:** `.map-label-btn` per serie (kleurt in de accentkleur + toont het aantal als de serie labels heeft). Klik → `openSerieLabelPopup(map, categorie, anchor)`: popup (hergebruikt de `.foto-label-popup`-stijl) met alle labels, vooringevuld uit `map.labels`, plus een veld om een nieuw label te maken (via het bestaande `POST /labels`). Expliciet 💾 Opslaan; knop + lokale `_mappenData` worden meteen bijgewerkt. `APP_VERSIE → 2026-07-23-b`.
- ☁️ **Worker:** nieuw endpoint `POST /fotograaf/serie-labels` (token-auth). Doet hetzelfde als labels kiezen bij uploaden, maar bewerkbaar (toevoegen én verwijderen): (1) werkt `map.labels` bij in `fotograaf:mappen:{id}`; (2) per foto `foto:labels` **mergen** (per-foto eigen labels blijven behouden, en verwijderen ruimt later netjes op via `verwijderFotoLabels`); (3) reverse index `label:fotos:{label}` per gewijzigd label in **één** read-modify-write met alle foto-keys van de serie (nooit per foto — zie KV-contentie-les v0.35). Gepagineerd via `lijstAlleR2`.
- ✅ **End-to-end geverifieerd via review-modus (als Jan), volledig omkeerbaar:** knop toont correct het labelaantal per serie; popup opent met de juiste labels vooraf geselecteerd (DTS → ZCFC + SV DTS). Testcyclus op DTS: tijdelijk "KFC" toegevoegd → opgeslagen → `map.labels` werd [ZCFC, SV DTS, KFC] en de KFC reverse-index groeide van 93→138 (DTS-foto's toegevoegd); daarna weer verwijderd → terug naar [ZCFC, SV DTS] en KFC-index terug op 93 (0 DTS). **Jans data exact hersteld, geen testlabel in de lijst achtergelaten** (bewust een bestaand label gebruikt i.p.v. een nieuw). Popup-persistentie bevestigd (heropenen toonde de opgeslagen keuze).
- 📖 fotograaf-handleiding.html: nieuwe stap "4b Labels toewijzen aan een serie" + 🏷-knop in de mockup.
- **Ontwerpnoot:** serie-labels volgen bewust exact de gast-upload-semantiek (per-foto entries in de reverse index, zodat clubs.html alle foto's toont zoals nu). Schrijfkosten ~N-foto's per bewerking, maar dit is een bewuste, laagfrequente actie (niet het request-pad).
