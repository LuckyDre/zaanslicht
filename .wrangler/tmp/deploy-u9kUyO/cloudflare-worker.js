...
async function getFotograafByToken(token, env) {
  ...
  if (!isReview) {
    ...
    if (Date.now() - laatstVerlengd > 24 * 3600 * 1e3) {
      try {
...
__name(handleLoginLog, "handleLoginLog");
...
var GH_REPO = "LuckyDre/zaanslicht";
var GH_BRANCH = "main";
...
async function githubGet(path, env) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`GitHub ${r.status}: ${msg.slice(0, 200)}`);
  }
  return r.json();
}
...
__name(handleAdminManifestGet, "handleAdminManifestGet");
...
__name(handleAdminManifestSave, "handleAdminManifestSave");
...
__name(handleAdminGithubFileGet, "handleAdminGithubFileGet");
...
__name(handleAdminGithubFilePut, "handleAdminGithubFilePut");
...
__name(handleReviewWachtwoord, "handleReviewWachtwoord");
...
__name(handleReviewSessie, "handleReviewSessie");
async function handleFotoUpload(request, env) {
  ...
  let labels = [];
  try {
    labels = JSON.parse(labelsRaw).slice(0, 10);
  } catch {
  }
  ...
  const isWebP = b[0] === 82 && b[1] === 73 && b[2] === 70 && b[3] === 70 && b[8] === 87 && b[9] === 69 && b[10] === 66 && b[11] === 80;
  ...
  if (thumb && thumb.size > 0) {
    ...
    const thumbIsWebP = tb[0] === 82 && tb[1] === 73 && tb[2] === 70 && tb[3] === 70 && tb[8] === 87 && tb[9] === 69 && tb[10] === 66 && tb[11] === 80;
    const thumbIsJPEG = tb[0] === 255 && tb[1] === 216 && tb[2] === 255;
...
async function handleFotosLijst(request, env) {
  ...
  const fotos = lijst.objects.filter((o) => isAdmin || !verborgenFotos.includes(o.key)).filter((o) => isAdmin || !verborgenMappen.some((m) => o.key.includes(`/${encodeURIComponent(m)}/`) || o.key.includes(`/${m}/`))).map((o) => ({
    key: o.key,
...
__name(handleFotoVolgorde, "handleFotoVolgorde");
...
__name(handleAdminMapVerwijderen, "handleAdminMapVerwijderen");
...
__name(handleAdminFotoVerwijderen, "handleAdminFotoVerwijderen");
...
__name(handleEigenVerborgenLijst, "handleEigenVerborgenLijst");
...
__name(handleEigenVerborgeMap, "handleEigenVerborgeMap");
...
__name(handlePositieBeheer, "handlePositieBeheer");
async function handleFotograafGalleryVolgorde(request, env) {
  ...
  if (huidig !== null) {
    ...
    if (JSON.stringify(andermansInNieuw.map((e) => e.type + e.map + (e.fgId || ""))) !== JSON.stringify(andermansInHuidig.map((e) => e.type + e.map + (e.fgId || "")))) {
...
__name(handleFotograafGalleryVolgorde, "handleFotograafGalleryVolgorde");
...
__name(handleMapDatum, "handleMapDatum");
...
__name(handleMapBeschrijving, "handleMapBeschrijving");
...
__name(handleMapNaam, "handleMapNaam");
async function handleMapPaginas(request, env) {
  ...
  const entry = mappen.find((m) => m.map === map);
  if (!entry) return json({ error: "map niet gevonden" }, 404);
  if (opVoetbal !== void 0) entry.opVoetbal = Boolean(opVoetbal);
  if (opNosports !== void 0) entry.opNosports = Boolean(opNosports);
  if (opEigenPagina !== void 0) entry.opEigenPagina = Boolean(opEigenPagina);
  await env.SUBSCRIBERS.put("fotograaf:mappen:" + fotograafId, JSON.stringify(mappen));
  return json({ ok: true });
}
__name(handleMapPaginas, "handleMapPaginas");
...
__name(handleGetAndreasProfile, "handleGetAndreasProfile");
...
__name(handleSetAndreasProfile, "handleSetAndreasProfile");
async function handleGetLabels(request, env) {
  ...
  const alle = [.../* @__PURE__ */ new Set([...STANDAARD_LABELS, ...opgeslagen])].filter((l) => !verwijderd.includes(l)).sort((a, b) => a.localeCompare(b, "nl"));
  return json({ labels: alle });
}
__name(handleGetLabels, "handleGetLabels");
...
__name(handleAddLabel, "handleAddLabel");
...
__name(updateReverseIndex, "updateReverseIndex");
...
__name(verwijderFotoLabels, "verwijderFotoLabels");
...
__name(handleGetFotoLabels, "handleGetFotoLabels");
async function handleSetFotoLabels(request, env) {
  ...
  if (isAdmin) {
    ...
    if (mapNaam) {
      entry.mapNaam = mapNaam;
      entry.type = entryType || "map";
      entry.cat = cat || "";
    }
  } else {
    const authToken = request.headers.get("X-Fotograaf-Token");
...
__name(handleSetFotoLabels, "handleSetFotoLabels");
...
__name(handleFotosBijLabel, "handleFotosBijLabel");
async function handleLabelsOpschonen(request, env) {
  if (!requireSecret(request, env)) return json({ error: "Geen toegang" }, 401);
  const uitvoeren = new URL(request.url).searchParams.get("uitvoeren") === "1";
  const bestaat = /* @__PURE__ */ new Set();
  let cursor;
  do {
    const res = await env.FOTOS.list({ prefix: "fotografen/", limit: 1e3, cursor });
    for (const o of res.objects) bestaat.add(o.key);
    cursor = res.truncated ? res.cursor : void 0;
  } while (cursor);
  const dood = /* @__PURE__ */ __name((key) => key.startsWith("fotografen/") && !bestaat.has(key), "dood");
  const indexReport = {};
  let indexCursor;
  do {
    const res = await env.SUBSCRIBERS.list({ prefix: "label:fotos:", cursor: indexCursor, limit: 1e3 });
    ...
    indexCursor = res.list_complete ? void 0 : res.cursor;
  } while (indexCursor);
  let fotoLabelsVerwijderd = 0;
  let flCursor;
  do {
    const res = await env.SUBSCRIBERS.list({ prefix: "foto:labels:", cursor: flCursor, limit: 1e3 });
    for (const k of res.keys) {
    ...
    flCursor = res.list_complete ? void 0 : res.cursor;
  } while (flCursor);
  return json({
...
__name(handleLabelsOpschonen, "handleLabelsOpschonen");
...
__name(handleMapRegistreren, "handleMapRegistreren");
...
__name(handleAlleComments, "handleAlleComments");
...
async function handleViewDashboard(request, env, id) {
  ...
  const filterByFotograaf = /* @__PURE__ */ __name((items) => items.filter((item) => item.fotos?.some((f) => f.fotograaf === id)), "filterByFotograaf");
  return json({
    account: {
      id: a.id,
      naam: a.naam,
...
__name(handleViewDashboard, "handleViewDashboard");
